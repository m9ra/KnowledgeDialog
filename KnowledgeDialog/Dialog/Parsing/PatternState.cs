...
namespace KnowledgeDialog.Dialog.Parsing
{
    class PatternState
    {
        ...
        public PatternState(UtterancePattern originalPattern)
        {
            OriginalPattern = originalPattern;
            Substitutions = new IEnumerable<string>[] { };
        }

        private PatternState(PatternState parent, int index, IEnumerable<IEnumerable<string>> substitutions)
        {
        ...
        internal IEnumerable<PatternState> GetNextStates(string inputWord)
        {
            ...
            {
                //keep at same index
                var sentenceState = new PatternState(this, StateIndex, sentenceSubstitutions);
                result.Add(sentenceState);
            }

            var nextWordState = new PatternState(this, StateIndex + 1, sentenceSubstitutions);
